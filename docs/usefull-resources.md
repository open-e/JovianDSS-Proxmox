...
Repozytorium zawiera projektowany plugin `sos` dla JovianDSS:

```text
tools/sos/joviandss.py
```

Do testów na hoście Proxmox z `sos >= 4.9.1` można skopiować go do katalogu
pluginów `sos`, zwykle:

```bash
cp tools/sos/joviandss.py /usr/share/sos/report/plugins/joviandss.py
```

Następnie można uruchomić tylko ten plugin:

```bash
sos report --only-plugins joviandss --cmd-timeout 300 --debug
```

Domyślnie plugin `sos` nie zbiera plików haseł z `/etc/pve/priv/storage/`.
Jeżeli podejrzewasz, że plik hasła ma zły format, można włączyć opcjonalne
zebranie zanonimizowanej wersji tych plików. Plugin odczyta pliki `.pw`, ale w
raporcie zapisze wartości haseł jako `<secret password>`, zachowując strukturę
linii.
