...
class CIFS():
    ...
    def __parse(self, args):
        ...
        cifs_parser.add_argument(
            'cifs_share_name', type=str, help='Name of CIFS share')

        parser = cifs_parser.add_subparsers(dest='cifs-action')

        ensure = parser.add_parser('ensure')
        ensure.add_argument('-s', '--size', dest='cifs_size',
                            default="10G", help='Size of CIFS share')
        ensure.add_argument('-u', '--user', required=True, dest='cifs_user',
                            help='Use specific user name for this share')
        ensure.add_argument('-p', '--password', required=True,
                            dest='cifs_password', help='Use specific password for user')
        ensure.add_argument('-n', '--nasname', required=True,
                            dest='cifs_nas_name', help='Use specific name for nas volume')

        extend = parser.add_parser('extend')
        extend.add_argument('cifs_share_name', type=str,
                            help='Name of CIFS share')
        extend.add_argument('-s', '--size', dest='cifs_size',
                            default="10G", help='New size of CIFS share')
    ...
    def ensure(self):
        ...
        try:
            ...
            self.jdss.ra.create_user(
                self.args['cifs_user'], self.args['cifs_password'])
        except Exception as err:
            raise err

        if user:
            self.jdss.ra.set_user_pass(
                self.args['cifs_user'], self.args['cifs_password'])
        ...
        if len(user_names) > 0:
            self.jdss.ra.delete_share_user(
                self.args['cifs_share_name'], user_names)

        self.jdss.ra.set_share_user(
            self.args['cifs_share_name'], self.args['cifs_user'])
